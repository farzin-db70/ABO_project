...
/* Private macro -------------------------------------------------------------*/
/* Private variables ---------------------------------------------------------*/
extern volatile uint8_t rxBuffer[100],rxCounter , recivePacketFlag;
...
if(UART1_GetFlagStatus(UART1_FLAG_RXNE) == 1)
  {
    ...
    }else if((rxCounter == 1) && (rxData != 0x06)){
      rxCounter=0;
    }else if((rxCounter > 0) && (rxData == '\n')){
      rxBuffer[rxCounter] =  rxData;
      rxCounter++;
      rxBuffer[rxCounter] = 0x00;
      recivePacketFlag=1;
    }else if(rxCounter > 0 && rxCounter<100){
      rxBuffer[rxCounter] =  rxData;
      rxCounter++;
    }else if(rxCounter >= 100){
      rxCounter=0;
    }
